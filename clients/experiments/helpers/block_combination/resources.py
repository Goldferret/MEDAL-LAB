"""
Resource creation and cleanup for block combination solver experiment.

Dynamically creates colored block resources (6 blocks of 4 colors) based on 
vision detection and manages their lifecycle.
"""
...
def create_block_resources(resource_client, location_client, colors, location_ids):
    ...
    Args:
        ...
        colors: List of detected colors (length 6)
    ...
    for i, color in enumerate(colors):
        ...
        # Create unique name by including position number
        # (since multiple blocks can have same color)
        block = Asset(
            resource_name=f"{color}_block_pos{i}",
        ...
        resource_ids[f"{color}_block_pos{i}"] = created.resource_id
